...
public abstract class Playable : StrategyObject {
	...
	public void Start()
	{
		...
		// Get animator from model with null check
    ...
    public void colorModel(Color c)
    {
        ...
        if (model != null && model.GetComponent<Renderer>() != null)
        {
            model.GetComponent<Renderer>().material.color = c;

            foreach (Transform t in model)
            {
                if (t != null && t.GetComponent<Renderer>() != null)
                {
    ...
    private void moving()
    {
        if (path != null)
        {
            ...
            if (path.vectorPath != null && currentWaypoint < path.vectorPath.Count)
            {
                ...
                if (currentWaypoint != null && currentWaypoint != 0 && currentWaypoint < path.vectorPath.Count)
                {
                    faceDirection(path.vectorPath[currentWaypoint]);
                }

                Vector3 move = transform.position + dir;
                transform.position = move;

                if (currentWaypoint != null && currentWaypoint < path.vectorPath.Count &&
                    (transform.position - path.vectorPath[currentWaypoint]).sqrMagnitude < 1)
                {
                    currentWaypoint++;
                }
